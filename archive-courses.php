...
get_header(); ?>
    ...
    <!-- Готово -->
    <section class="category relative z-0 bg-cover bg-no-repeat bg-center pt-32 pb-16 mb-24 sm:mb-32 min-h-[680px] sm:min-h-[90vh] lg:pt-[230px] lg:pb-48" style="background-image: url(<?php the_field('courses_bg'); ?>);">
      <div class="mx-auto container px-3 sm:px-6 lg:px-16">
        <div class="content max-lg:flex max-lg:flex-col max-lg:items-center max-lg:justify-center">
          <h1 class="text-white uppercase font-medium max-w-6xl text-3xl leading-9 mb-7 max-lg:mb-60 max-lg:text-center md:text-6xl md:leading-[66px] lg:mb-10 2xl:text-8xl 2xl:leading-[100px]"><?php the_field('courses_title'); ?></h1>
          <div class="max-lg:text-center text-lg leading-6 text-white max-w-2xl max-lg:order-2 2xl:text-xl 2xl:leading-8 lg:mb-16"><?php the_field('courses_text'); ?></div>
          <?php $courses_booknow_link = get_field('courses_booknow_link');
          if( $courses_booknow_link ): 
              $courses_booknow_link_url = $courses_booknow_link['url'];
              $courses_booknow_link_title = $courses_booknow_link['title'];
              $courses_booknow_link_target = $courses_booknow_link['target'] ? $courses_booknow_link['target'] : '_self';
              ?>
            <div class="flex gap-x-10 max-lg:order-1 max-lg:mb-7">
              <a href="<?php echo esc_url( $courses_booknow_link_url ); ?>" target="<?php echo esc_attr( $courses_booknow_link_target ); ?>" class="inline-flex items-center justify-center gap-x-6 py-6 px-8 text-lg leading-5 font-medium text-white rounded-full border-white border border-opacity-20 backdrop-blur-lg bg-card lg:text-xl lg:leading-6 hover:shadow-myShadow1"><?php echo esc_html( $courses_booknow_link_title ); ?><i class="icomoon icon-whatsapp text-2xl leading-none font-normal"></i></a>
            </div>
          <?php endif; ?>
        </div>
      </div>
    ...
    <section class="standartbl mb-32">
      <div class="mx-auto container px-3 sm:px-6 lg:px-16">
        <div class="flex max-lg:flex-col max-lg:justify-center gap-y-5 items-center gap-x-7 mb-6 lg:items-end">
          <h2 class="text-6xl text-white font-medium max-lg:text-center text-opacity-5 uppercase lg:text-5xl lg:text-opacity-15 xl:text-9xl 2xl:text-[10rem] max-2xl:leading-none"><?php the_field('coursesbl_title'); ?></h2>
        </div>
            ...
            if( $posty_kursov ): ?>
              <?php foreach( $posty_kursov as $post): // Переменная должна быть названа обязательно $post (IMPORTANT) ?>
                ...
                <div class="card-wrapper relative">
                  <a href="<?php the_permalink(); ?>" class="z-0 card transition-all flex flex-col justify-between relative w-full py-7 px-9 min-h-[540px] lg:justify-end lg:p-12 lg:min-h-[660px] 3xl:min-h-[740px]">
      ...
      <div class="mx-auto container px-3 sm:px-6 lg:px-16">
        <div class="relative flex flex-col justify-between flex-wrap gap-y-10 max-lg:items-center lg:flex-row">
          <div class="w-full xl:w-6/12">
            <h2 class="text-3xl leading-8 font-medium text-white uppercase mb-12 max-lg:text-center sm:text-5xl sm:leading-[1.1] lg:text-6xl lg:leading-[1.1]"><?php the_field('courses_cta_title'); ?></h2>
          ...
          <div class="w-full p-7 text-white border-white border border-opacity-20 backdrop-blur-lg bg-card h-fit lg:p-12 lg:pb-20 xl:w-5/12">
            <?php echo do_shortcode('[contact-form-7 id="314" title="Form Courses"]'); ?>
          </div>
        </div>
      </div>
    </section>
